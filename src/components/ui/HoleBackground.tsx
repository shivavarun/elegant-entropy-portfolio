...
function HoleBackground({
  ...
  const setParticles = React.useCallback(() => {
    ...
    const totalParticles = 30; // Further reduced for ultra-light performance
  ...
  const moveDiscs = React.useCallback(() => {
    stateRef.current.discs.forEach((disc: any) => {
      disc.p = (disc.p + 0.0003) % 1; // Even slower for ultra-light performance
      tweenDisc(disc);
    });
  }, [tweenDisc]);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'before:content-[""] before:absolute before:top-1/2 before:left-1/2 before:block before:size-[140%] dark:before:[background:radial-gradient(ellipse_at_50%_55%,transparent_10%,black_50%)] before:[background:radial-gradient(ellipse_at_50%_55%,transparent_10%,white_50%)] before:[transform:translate(-50%,-50%)]',
        'after:content-[""] after:absolute after:z-[5] after:top-1/2 after:left-1/2 after:block after:size-full after:[background:radial-gradient(ellipse_at_50%_75%,#a900ff_20%,transparent_75%)] after:[transform:translate(-50%,-50%)] after:mix-blend-overlay',
        className,
      )}
      {...props}
    >
      ...
      <motion.div
        className={cn(
          'absolute top-[-71.5%] left-1/2 z-[3] w-[30%] h-[140%] rounded-b-full blur-xl opacity-60 dark:mix-blend-plus-lighter mix-blend-plus-darker [transform:translate(-50%,0)] [background-position:0%_100%] [background-size:100%_200%]', // Reduced blur and opacity for lighter performance
        ...
        transition={{ duration: 12, ease: 'linear', repeat: Infinity }} // Even slower for ultra-light animation
      />
